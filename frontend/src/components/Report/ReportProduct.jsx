...
const ReportProduct = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${server}/report/report-product/${productId}`, data);
  ...
  return (
    <div className="flex justify-center p-6 bg-gray-100">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg w-full">
        <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">Report a Product</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="productId" className="block text-left font-semibold text-gray-700 mb-2">Product ID</label>
            <input
              type="text"
              id="productId"
              value={productId}
          ...
          <div>
            <label htmlFor="reason" className="block text-left font-semibold text-gray-700 mb-2">Reason for Report</label>
            <select
              id="reason"
              value={reason}
          ...
          {reason === 'Other' && (
            <div>
              <label htmlFor="otherReason" className="block text-left font-semibold text-gray-700 mb-2">State your reason</label>
              <input
                type="text"
                id="otherReason"
                value={otherReason}
